...
Wi-Fi, which can in turn be fixed in config.txt. It it is not fixed in config.txt, it follows the
...
the central's broadcast. This does not fit well with peripherals that a) deep sleep and
...
timestamp and could be a vector of a DoS attack. The central device may choose to throttle
...
blind to broadcast. When a central device sees this packet, it broadcasts the timestamp
immediately, to be hopefully detected by the peripheral.
...
channel every 5s and sending pair req packets every 1s.
The profile may set a timeout (e.g. 5 minutes) to quit and go back to sleep.

Once the peripheral gets a valid timestamp packet, it adopts the central that sent it as the
