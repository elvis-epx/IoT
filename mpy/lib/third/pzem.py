# Based on PZEM v3.0 driver by Jacopo Rodeschini

import ustruct as struct
import time

class PZEM:

    # Default single slave address [usefull to set specific address] (see doc)
    addr = 0xF8

    # Device command function admitted (see modbus protocl)
    CMD_RHR = 0x03  # READ HOLDING REGISTRY
    CMD_RIR = 0x04  # READ INPUT REGISTRY
    CMD_WSR = 0x06  # WRITE SINGLE REGISTRY

    # Reading values
    Voltage = 0
    Current = 0
    ActivePower = 0
    ActiveEnergy = 0
    Frequency = 0
    PowerFactor = 0
    Allarms = 0
    threshold = 0

    def __init__(self, uart, addr=0xF8):
        # set uart & update field
        self.uart = uart
        self.uart.init(bits=8, parity=None, stop=1, baudrate=9600, timeout=1)
        self.addr = addr

    def check_addr_start(self):
        self.read_addr_start()

    def get_addr(self):
        return self.addr

    def set_addr_start(self, addr):
        self.sendCommand(cmd=0x06, regAddr=0x02, opt=addr, buf=8)

    def read_addr_start(self):
        self.sendCommand(cmd=0x03, regAddr=0x02, opt=0x01, buf=7)

    def read_energy_start(self):
        return self.sendCommand(cmd=0x04, regAddr=0x00, opt=0x0A, buf=25)

    def reset_energy_start(self):
        return self.sendCommand(cmd=0x42, buf=4)

    def sendCommand(self, cmd=0x04, regAddr=None, opt=None, buf=25):
        """Send command to PZEM device & wait for the response

        Args:
            cmd       (byte): Command to send (0x03, 0x04, 0x06)
            regAddr   (byte): Start reading register low byte address
            opt       (byte): Number of register to read (or the value
                            for set the register)
            buf       (int) : Number of byte expected in the reply message

        Returns:
            (bool): reding status
        """

        # Start to build hex string command
        # (> = big endian formst)
        # (B = Unsigned char, 1 byte)
        # (H = Unsigned short, 2 byte)
        # [BBhh = 6 bytes]
        if cmd == 0x42:  # Reset the energy cmd
            frame = struct.pack(">BB", self.addr, cmd)
        else:
            frame = struct.pack(">BBHH", self.addr, cmd, regAddr, opt)

        # Compute the CRC of frame (2 bytes (high & low))
        crc16 = self.getCRC16(frame)
        crc_h = crc16 >> 8 & 0xFF
        crc_l = crc16 & 0xFF

        # Add crc to frame obj
        if cmd == 0x42:
            frame = struct.pack(">BBBB", self.addr, cmd, crc_l, crc_h)
        else:
            frame = struct.pack(
                ">BBHHBB", self.addr, cmd, regAddr, opt, crc_l, crc_h
            )

        # Send frame to the UART port
        self.uart.write(frame)
        self.buf = buf
        self.regAddr = regAddr

    def complete_trans(self):
        # Read the response, maximum 25 bytes
        # (25 bytes = (2 * 10 + 1 + 1 + 1 ) + 2 CRC )
        rcvFrame = self.uart.read()
        if not rcvFrame:
            return False
        frame = list(rcvFrame)

        return len(frame) == self.buf \
            and self.checkCRC16(frame) \
            and self.checkResponse(frame) \
            and self.updateValue(frame=frame, reg=self.regAddr)

    def getCRC16(self, frame):
        crc = 0xFFFF
        for ch in frame:
            crc = (crc >> 8) ^ self.table[(crc ^ ch) & 0xFF]
        return crc

    def checkCRC16(self, frame):
        CRC_high = frame.pop()
        CRC_low = frame.pop()

        # Check CRC
        crc = self.getCRC16(frame)
        return (CRC_high << 8 | CRC_low) == crc

    def checkResponse(self, frame):
        return not (frame[1] == 0x84 or frame[1] == 0x86 or frame[1] == 0xC2)

    def updateValue(self, frame, reg=None):
        """Update the measurement result after self.read() function is called.

        Args:
            frame (list of byte): list of received byte

        Returns:
            (bool): return true if the values are correctly update
        """

        try:
            # Update the measurement value (input registry, cmd = 0x04)
            if frame[1] == 0x04:

                self.Voltage = (frame[3] << 8 | frame[4]) / 10
                self.Current = (
                    frame[5] << 8 | frame[6] | frame[7] << 24 | frame[8] << 16
                ) / 1000
                self.ActivePower = (
                    frame[9] << 8 | frame[10] | frame[11] << 24 | frame[12] << 16
                ) / 10
                self.ActiveEnergy = (
                    frame[13] << 8 | frame[14] | frame[15] << 24 | frame[16] << 16
                )
                self.Frequency = (frame[17] << 8 | frame[18]) / 10
                self.PowerFactor = (frame[19] << 8 | frame[20]) / 100
                self.Allarms = frame[21] << 8 | frame[22]

            # Read the allarm threshold value
            # (holding registry = 0x0001, cmd = 0x03)
            elif frame[1] == 0x03 and reg == 0x01:
                self.threshold = frame[3] << 8 | frame[4]

            # Read the Modbus-RTU address
            # (holding registry = 0x0002, cmd = 0x03)
            elif frame[1] == 0x03 and reg == 0x02:
                self.addr = frame[4]

            # Update the allarm threshold value
            # (holding registry 0x0001, cmd = 0x06)
            elif frame[1] == 0x06 and frame[3] == 0x01:
                self.threshold = frame[4] << 8 | frame[5]

            # Update allarm threshold value
            # (holding registry 0x0002, cmd = 0x06)
            elif frame[1] == 0x06 and frame[3] == 0x02:
                self.addr = frame[5]

        except:
            return False
        else:
            return True

    def get_data(self):
        return {"V": self.Voltage,
                "A": self.Current,
                "W": self.ActivePower,
                "Wh": self.ActiveEnergy,
                "Hz": self.Frequency,
                "pf": self.PowerFactor}

    # Default ModBus-16 CRC Table
    table = (
        0x0000,
        0xC0C1,
        0xC181,
        0x0140,
        0xC301,
        0x03C0,
        0x0280,
        0xC241,
        0xC601,
        0x06C0,
        0x0780,
        0xC741,
        0x0500,
        0xC5C1,
        0xC481,
        0x0440,
        0xCC01,
        0x0CC0,
        0x0D80,
        0xCD41,
        0x0F00,
        0xCFC1,
        0xCE81,
        0x0E40,
        0x0A00,
        0xCAC1,
        0xCB81,
        0x0B40,
        0xC901,
        0x09C0,
        0x0880,
        0xC841,
        0xD801,
        0x18C0,
        0x1980,
        0xD941,
        0x1B00,
        0xDBC1,
        0xDA81,
        0x1A40,
        0x1E00,
        0xDEC1,
        0xDF81,
        0x1F40,
        0xDD01,
        0x1DC0,
        0x1C80,
        0xDC41,
        0x1400,
        0xD4C1,
        0xD581,
        0x1540,
        0xD701,
        0x17C0,
        0x1680,
        0xD641,
        0xD201,
        0x12C0,
        0x1380,
        0xD341,
        0x1100,
        0xD1C1,
        0xD081,
        0x1040,
        0xF001,
        0x30C0,
        0x3180,
        0xF141,
        0x3300,
        0xF3C1,
        0xF281,
        0x3240,
        0x3600,
        0xF6C1,
        0xF781,
        0x3740,
        0xF501,
        0x35C0,
        0x3480,
        0xF441,
        0x3C00,
        0xFCC1,
        0xFD81,
        0x3D40,
        0xFF01,
        0x3FC0,
        0x3E80,
        0xFE41,
        0xFA01,
        0x3AC0,
        0x3B80,
        0xFB41,
        0x3900,
        0xF9C1,
        0xF881,
        0x3840,
        0x2800,
        0xE8C1,
        0xE981,
        0x2940,
        0xEB01,
        0x2BC0,
        0x2A80,
        0xEA41,
        0xEE01,
        0x2EC0,
        0x2F80,
        0xEF41,
        0x2D00,
        0xEDC1,
        0xEC81,
        0x2C40,
        0xE401,
        0x24C0,
        0x2580,
        0xE541,
        0x2700,
        0xE7C1,
        0xE681,
        0x2640,
        0x2200,
        0xE2C1,
        0xE381,
        0x2340,
        0xE101,
        0x21C0,
        0x2080,
        0xE041,
        0xA001,
        0x60C0,
        0x6180,
        0xA141,
        0x6300,
        0xA3C1,
        0xA281,
        0x6240,
        0x6600,
        0xA6C1,
        0xA781,
        0x6740,
        0xA501,
        0x65C0,
        0x6480,
        0xA441,
        0x6C00,
        0xACC1,
        0xAD81,
        0x6D40,
        0xAF01,
        0x6FC0,
        0x6E80,
        0xAE41,
        0xAA01,
        0x6AC0,
        0x6B80,
        0xAB41,
        0x6900,
        0xA9C1,
        0xA881,
        0x6840,
        0x7800,
        0xB8C1,
        0xB981,
        0x7940,
        0xBB01,
        0x7BC0,
        0x7A80,
        0xBA41,
        0xBE01,
        0x7EC0,
        0x7F80,
        0xBF41,
        0x7D00,
        0xBDC1,
        0xBC81,
        0x7C40,
        0xB401,
        0x74C0,
        0x7580,
        0xB541,
        0x7700,
        0xB7C1,
        0xB681,
        0x7640,
        0x7200,
        0xB2C1,
        0xB381,
        0x7340,
        0xB101,
        0x71C0,
        0x7080,
        0xB041,
        0x5000,
        0x90C1,
        0x9181,
        0x5140,
        0x9301,
        0x53C0,
        0x5280,
        0x9241,
        0x9601,
        0x56C0,
        0x5780,
        0x9741,
        0x5500,
        0x95C1,
        0x9481,
        0x5440,
        0x9C01,
        0x5CC0,
        0x5D80,
        0x9D41,
        0x5F00,
        0x9FC1,
        0x9E81,
        0x5E40,
        0x5A00,
        0x9AC1,
        0x9B81,
        0x5B40,
        0x9901,
        0x59C0,
        0x5880,
        0x9841,
        0x8801,
        0x48C0,
        0x4980,
        0x8941,
        0x4B00,
        0x8BC1,
        0x8A81,
        0x4A40,
        0x4E00,
        0x8EC1,
        0x8F81,
        0x4F40,
        0x8D01,
        0x4DC0,
        0x4C80,
        0x8C41,
        0x4400,
        0x84C1,
        0x8581,
        0x4540,
        0x8701,
        0x47C0,
        0x4680,
        0x8641,
        0x8201,
        0x42C0,
        0x4380,
        0x8341,
        0x4100,
        0x81C1,
        0x8081,
        0x4040,
    )
